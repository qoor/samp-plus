...
CMultiplayerSA::CMultiplayerSA()
{
	...
	Debug::write("CMultiplayerSA 생성자 로드 됨.");
}
...
void CMultiplayerSA::InitHooks()
{
    ...
    MemSet((void*)0x442AD0, 0xC3, 1);*/ // 적용 시 플레이어가 죽은 후 리스폰 되지 않음.
    ...
    //HookInstall(HOOKPOS_CRunningScript_Process, (DWORD)HOOK_CRunningScript_Process, 6); // 확실히 문제 있음.
    HookInstall(HOOKPOS_CExplosion_AddExplosion, (DWORD)HOOK_CExplosion_AddExplosion, 6);
    HookInstall(HOOKPOS_CRealTimeShadowManager__ReturnRealTimeShadow, (DWORD)HOOK_CRealTimeShadowManager__ReturnRealTimeShadow, 6); // 여기까진 안전
    ...
    HookInstall(HOOKPOS_CTaskComplexJump__CreateSubTask, (DWORD)HOOK_CTaskComplexJump__CreateSubTask, 6); // 여기까진 안전
    ...
    HookInstall(HOOKPOS_CCam_ProcessFixed, (DWORD)HOOK_CCam_ProcessFixed, 5); // 여기까진 안전
    //HookInstall(HOOKPOS_CTaskSimplePlayerOnFoot_ProcessPlayerWeapon, (DWORD)HOOK_CTaskSimplePlayerOnFoot_ProcessPlayerWeapon, 7); // MTA Sync 관련 후킹이므로 필요 없음.
    HookInstall(HOOKPOS_CPed_IsPlayer, (DWORD)HOOK_CPed_IsPlayer, 6); // 여기까진 안전
    HookInstall(HOOKPOS_CTrain_ProcessControl_Derail, (DWORD)HOOK_CTrain_ProcessControl_Derail, 6);
    HookInstall(HOOKPOS_CVehicle_SetupRender, (DWORD)HOOK_CVehicle_SetupRender, 5); // 문제 있을 가능성 있음. Alpha값이 변경될 경우 어떻게 될지 모름.
    ...
    HookInstall(HOOKPOS_CWorld_ProcessVerticalLineSectorList, (DWORD)HOOK_CWorld_ProcessVerticalLineSectorList, 8); // 여기까진 안전
    ...
    /*HookInstall(HOOKPOS_OccupiedVehicleBurnCheck, (DWORD)HOOK_OccupiedVehicleBurnCheck, 6); // 문제 생길 여지 있음. 차량이 월드에 존재하는데 플러그인에서는 등록하지 못한 경우.
    HookInstall(HOOKPOS_UnoccupiedVehicleBurnCheck, (DWORD)HOOK_UnoccupiedVehicleBurnCheck, 5); // 상동*/
    ...
    HookInstall(HOOKPOS_CGame_Process, (DWORD)HOOK_CGame_Process, 10); // 여기까진 안전
    //HookInstall(HOOKPOS_Idle, (DWORD)HOOK_Idle, 10); // MTA Idle 프로세스를 위한 것이기 때문에 필요 없음.
    ...
    HookInstall(HOOKPOS_CHandlingData_isNotFWD, (DWORD)HOOK_isVehDriveTypeNotFWD, 7); // 여기까진 안전
	...
	*(reinterpret_cast<BYTE*>(0x00BAB318)) = 0; *(reinterpret_cast<BYTE*>(0x0053E94C)) = 0;
	...
	MemPut<BYTE>(0x63F576, 0xEB);*/ // 주석 고정
    ...
    SetSuspensionEnabled(true); // 차량 핸들링 데이터를 등록하지 않기(CVehicle 생성자 후킹 참조) 때문에 서스펜션 후킹을 끔.
	...
	Debug::write("CMultiplayerSA 후킹 적용 됨.");
}
...
void __cdecl ResetVehicleHandlingData(CVehicle* targetVehicle)
{
	if (targetVehicle == NULL)
		return;

	const CHandlingEntry* originalHandlingEntry = core->getGame()->GetHandlingManager()->GetOriginalHandlingData((eVehicleTypes)targetVehicle->GetModelIndex());
	CHandlingEntry* newHandlingEntry = core->getGame()->GetHandlingManager()->CreateHandlingData();

	newHandlingEntry->Assign(originalHandlingEntry);
	targetVehicle->SetHandlingData(newHandlingEntry);
}
...
void                        GetVehicleDriveType()
{
	...
	CVehicle* targetVehicle = pHandlingDriveTypeVeh->m_pVehicle;

	if (targetVehicle != NULL)
	{
		CHandlingEntry* handlingEntry = targetVehicle->GetHandlingData();

		if (handlingEntry == NULL)
		{
			ResetVehicleHandlingData(targetVehicle);
			handlingEntry = targetVehicle->GetHandlingData();

			if (handlingEntry == NULL)
				return;
		}

		ucDriveType = static_cast<unsigned char>(handlingEntry->GetCarDriveType());
	}
}
...
{
	TIMING_CHECKPOINT("+ CPhysical_ApplyGravity");
    DWORD dwType;
    _asm
    {
        mov ecx, dwThis
...
{
	TIMING_CHECKPOINT("+ VehicleCamStart()");
...
{
	TIMING_CHECKPOINT("+ SaveVehicleColors()");
    CVehicle* pVehicle = pGameInterface->GetPools()->GetVehicle((DWORD*)dwThis);
    if (pVehicle)
    {
        pVehicle->GetColor(&vehColors[0], &vehColors[1], &vehColors[2], &vehColors[3], true);

		if (vehColors[0].ulARGB == 0 && vehColors[1].ulARGB == 0 && vehColors[2].ulARGB == 0 && vehColors[3].ulARGB == 0)
		{
			CVehicleSAInterface* vehicleInterface = (CVehicleSAInterface*)dwThis;

			if (vehicleInterface->m_colour1 != 0 || vehicleInterface->m_colour2 != 0 || vehicleInterface->m_colour3 != 0 || vehicleInterface->m_colour4 != 0)
			{
				vehColors[0] = CVehicleColor::GetRGBFromPaletteIndex(vehicleInterface->m_colour1);
				vehColors[1] = CVehicleColor::GetRGBFromPaletteIndex(vehicleInterface->m_colour2);
				vehColors[2] = CVehicleColor::GetRGBFromPaletteIndex(vehicleInterface->m_colour3);
				vehColors[3] = CVehicleColor::GetRGBFromPaletteIndex(vehicleInterface->m_colour4);

				//pVehicle->SetColor(vehColors[0], vehColors[2], vehColors[3], vehColors[4], 0);
			}
		}
    }
	TIMING_CHECKPOINT("- SaveVehicleColors()");
...
bool                 CheckHasSuspensionChanged(void)
{
    ...
    if (pSuspensionInterface)
    {
		...
		{
			CHandlingEntry* vehicleHandling = pVehicle->GetHandlingData();

			if (vehicleHandling == NULL)
				ResetVehicleHandlingData(pVehicle);
