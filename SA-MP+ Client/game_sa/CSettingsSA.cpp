...
{
    float fFieldOfView = 70.0;
    g_pCore->getSettings()->getSettingData("FieldOfView", fFieldOfView);
...
void CSettingsSA::ResetVehiclesLODDistance(void)
{
    ...
    g_pCore->getSettings()->getSettingData("HighDetailVehicles", bHighDetailVehicles);
...
int CSettingsSA::OnSelectDevice(void)
{
    ...
    if (GetNumAdapters() > 1)
    {
		Debug::write("CSettingsSA* OnSelectDevice() return 2");
        // Hide device selection
        return 2;
    }
    else
    {
		Debug::write("CSettingsSA* OnSelectDevice return 0");
        return 0;
    }
}
...
__declspec(noinline) int OnMY_SelectDevice(void)
{
	...
	Debug::write("CSettingsSA* OnMY_SelectDevice()");
