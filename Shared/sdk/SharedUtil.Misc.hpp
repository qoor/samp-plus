...
{
    return RegDeleteKeyA(hkRoot, szSubKey) == ERROR_SUCCESS;
}
...
void SharedUtil::AddExceptionReportLog(uint uiId, const char* szExceptionName, const char* szExceptionText)
{
    ...
    OutputDebugStringA("[ReportLog] ");
    OutputDebugStringA(&szOutput[0]);
                           ...
                           const SString& strDirectory = "", int nShowCmd = SW_SHOWNORMAL)
{
    ...
    {
        int iResult = (int)ShellExecuteA(NULL, strAction, strFile, strParameters, strDirectory, nShowCmd);
        return iResult > 32;
    }
}
