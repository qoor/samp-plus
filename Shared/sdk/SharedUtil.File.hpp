...
{
#ifdef WIN32
    DWORD dwAtr = GetFileAttributesA(strFilename);
...
{
#ifdef WIN32
    DWORD dwAtr = GetFileAttributesA(strPath);
    if (dwAtr == INVALID_FILE_ATTRIBUTES)
        return false;
    return ((dwAtr & FILE_ATTRIBUTE_DIRECTORY) != 0);
#else
...
#ifdef WIN32
    if (bForce)
        SetFileAttributesA(strFilename, FILE_ATTRIBUTE_NORMAL);
#endif
...
{
#ifdef WIN32
    return MoveFileA(strFilenameOld, strFilenameNew) != 0;
#else
...
#ifdef WIN32
    if (bForce)
        SetFileAttributesA(strDest, FILE_ATTRIBUTE_NORMAL);
#endif
...
SString SharedUtil::MakeUniquePath(const SString& strInPathFilename)
{
    ...
    int     iCount = 1;
#ifdef WIN32
    while (GetFileAttributesA(strTest) != INVALID_FILE_ATTRIBUTES)
#else
